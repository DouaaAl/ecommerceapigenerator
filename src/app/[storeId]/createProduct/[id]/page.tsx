...
const page = () => {
    ...
    const updateProduct = async(e: any) =>{
        ...
        const newproduct = await updateProductAPI(name, featured, mappedSizes, mappedImages, mappedCategories, mappedColors, storeId, quantity, price, description, productId);
    ...
    const addColor =(e: any) =>{
        e.preventDefault();
        setData({...data, Colors:[...data?.Color, currentColor].filter(onlyUnique)})
    }
    ...
    const getProduct = async()=>{
        setLoading(true);
      const res = await getProductAPI(storeId, productId);
      let productImages: any[] = [];
      res.Images.forEach((image: any)=>{
        productImages.push(image?.url);
      })
      let productSizes: any[] = [];
      res.Sizes.forEach((size: any)=>{
        productSizes.push(size?.id)
      })
      ...
      res.Color.forEach((color:any)=>{
        productColors.push(color.id);
      })

      setImages(productImages);
      setData({Sizes: productSizes, Color: productColors, Categories: productCategories})
      setName(res.name);
